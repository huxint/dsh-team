/**
 * What is screwed to the walls and hung from the ceiling: the calendar, the
 * whiteboard, the hanging plant, the shelf and its books, the clock, the air
 * conditioner, and the pendant lamps.
 *
 * Every fixture is placed by the SAME floor coordinate a member would stand at
 * to look at it, so the window somebody wanders over to is the window they end
 * up under. The wall itself belongs to the shell; this is only what is on it.
 */
...
/** Where the pendants hang, in world units across and into the room. */
...
/** How far below the ceiling a pendant's bulb hangs. */
export const PENDANT_DROP = 0.98

/** What the fixtures hand back: the group, and the pendants by themselves so they can sway. */
...
/**
 * Everything on the walls and the ceiling.
 * @param shop - where the materials come from.
 * @returns the fixtures; wall pieces on the shell layer, pendants on the furniture layer.
 */
...
/** The wall calendar, on the month somebody last turned it to. */
function calendar(shop: Shop, x: number): Group {
  ...
  group.add(box(0.34, 0.44, 0.012, shop.matte(p.paper, { map, roughness: 0.9 }), { x, y: 2.02, z: BACK + 0.006 }, { cast: false }))
  group.add(sphere(0.012, shop.matte(p.metalDark, { roughness: 0.4, metalness: 0.6 }), { x, y: 2.25, z: BACK + 0.012 }, { cast: false }))
  return group
}

/** The whiteboard: framed, with a tray of markers under it. */
function whiteboard(shop: Shop, x: number): Group {
  ...
  group.add(box(width, height, 0.02, shop.matte(p.paper, { map, roughness: 0.4 }), { x, y, z: BACK + 0.02 }, { cast: false }))
...
/** A pothos in a bracket, trailing down the wall beside the whiteboard. */
...
/** A plank on two brackets: books, a trophy nobody remembers winning, a cactus. */
function shelf(shop: Shop, x: number): Group {
  ...
  // Seven spines of different heights, tallest first, and one that fell over.
  ...
  // The cactus at the end of the plank.
...
/** The wall clock, stopped at ten past ten. */
function clock(shop: Shop, x: number): Group {
  ...
  group.add(piece(new CircleGeometry(0.2, 40), shop.matte(p.paper, { map: face, roughness: 0.6 }), { x, y, z: BACK + 0.046 }, { cast: false, receive: false }))
  group.add(ring(0.205, 0.012, iron, { x, y, z: BACK + 0.046, rx: 0 }, { cast: false }))
  return group
}

/** The split air conditioner at the wall's right end. */
...
/** One pendant lamp, hung from the ceiling on a cord; its pivot is at the ceiling so it can sway. */
function pendant(shop: Shop, x: number, z: number): Group {
  ...
  shade.name = 'shade'
  group.add(shade)
  const bulb = sphere(0.045, shop.matte(p.bulb, { emissive: p.bulb, emissiveIntensity: 0.4, roughness: 0.3 }), { y: -PENDANT_DROP + 0.05 }, { cast: false, receive: false })
  bulb.name = 'bulb'
  group.add(bulb)
  return group
}
